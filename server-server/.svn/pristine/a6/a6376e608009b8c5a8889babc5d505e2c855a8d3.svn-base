...
public class MyMessageEncoder implements MessageEncoder<AbstractMessage> {
	...
	public void encode(IoSession session, AbstractMessage message,
		...
		case AbstractMessage.INTERNAL_QUERY:  //查询信息结果反馈
			...
			out.write(buf);
			break;
		case AbstractMessage.INTERNAL_MONITOR: //监控信息反馈
			MonitorMessage mm = (MonitorMessage)message;
			buf.put(AbstractMessage.HEAD);//把YJ放进缓冲区
			buf.put(AbstractMessage.INTERNAL_MONITOR);
			byte[] bb = mm.getMonitor().getBytes("utf-8");
			buf.put((byte)bb.length);
			buf.put(bb);
			buf.put((byte)0x01);
			buf.flip();
			out.write(buf);
