...
public class CommunicateService {
	...
	public void removeSession(IoSession session){
	 ...
	 public void processQueryMessage(IoSession session, QuerMessage msg){
		 ...
		 Scanner s;
		 if(query.startsWith("ls")){    //查询详细设备信息
			 	s = new Scanner(query);
			 	s.next();
			 	if(s.hasNext()){
					...
					//如果根据家庭域名没有查询到信息  则根据设备ID来查询
					if(device == null){
						List<DeviceVo> list =   deviceService.queryDevicesByDeviceId(id);
						if(list != null && list.size()>0)
							device = list.get(0);
					}
					if (device != null) {
						if(gateWayMap.get(device.getDeviceId()) != null)
							msg.setQueryMsg("设备信息[L]:"+device.toQueryString());
						else
							msg.setQueryMsg("设备信息[S]:"+device.toQueryString());
					} else
						msg.setQueryMsg("["+id+"]设备不存在！");
					session.write(msg);
					return;
			 	}
		 }else if(query.equals("count online")){   //查询当前在线网关数
			 try{
			 ...
			 session.write(msg);
			 return;
		 }else if(query.equals("count gateway")){  //查询当前与服务器建立长连接的网关数
			 int num = gateWayMap.size();
			 msg.setQueryMsg("当前与服务器建立长连接网关数:"+num);
			 session.write(msg);
			 return;
		 }else if(query.equals("count client")){   //查询大年与服务器建立长连接的客户端数
			 int num = clientMap.size();
			 msg.setQueryMsg("当前与服务器建立长连接客户端数:"+num);
			 session.write(msg);
			 return;
		 }else if(query.equals("login")){   //服务器监控软件登录
			 newMonitorSession(session);
			 logger.info("New monitor session:"+session);
			 return;
		 }else if("exit".equals(query)){  //服务器监控软件退出
			 session.close(true);
			 return;
		 }else if(("online gateway").equals(query)){  //查询长连接在线网关
				synchronized (gateWayMap) {
					...
					while(it.hasNext()){
						ss = it.next();
						obj = ss.getAttribute(AUTHORIZATION);
						if(obj != null){
							auth = (Authorization)obj;
							msg.setQueryMsg(auth.getHostName()+"\t"+auth.getDeviceId());
							session.write(msg);
						}
					}
				}
			 return;
		 }else if("online client".equals(query)){  //查询长连接客户端信息
			 synchronized (clientMap) {
					...
					while(it.hasNext()){
						...
						if(list!=null && list.size()!=0){
							for(int i=0;i<list.size();i++){
								...
								if(obj != null){
									auth = (Authorization)obj;
									msg.setQueryMsg(auth.getHostName()+"\t"+auth.getUserName()+"\t"+auth.getDeviceId());
									session.write(msg);
								}
							}
						}
